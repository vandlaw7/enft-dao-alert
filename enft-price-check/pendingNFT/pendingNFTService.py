...
from pendingNFTRepository import pendingNFTRepository
from publicAccount.pubAccRepository import publicAccountRepository
...
def pendingNFTService():
    ...
    elif (voteRes['nftId'] not in nftlist) & (voteRes['answer'] == 'buy'):
        # 실제 DB에서는 insert SQL을 써야함!!!!
        # 그리고, buyLimitTokenAmt는 정책에 따라 변수로 변경될 수 있음.
        appendItem = {
            'nftAddress': voteRes['nftId'],
            'source': voteRes['source'],
            'type': voteRes['type'],
            'approvalTokenAmt': updateTokenAmt,
            'buyLimitTokenAmt': 250
        }
        pendingNFTRepository.append(appendItem)
        if updateTokenAmt > appendItem['buyLimitTokenAmt']:
            approvalProcess()
